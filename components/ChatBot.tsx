    ...
    <div className="flex flex-col h-full bg-black">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800/50 bg-zinc-900/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-[10px] text-pink-400 font-bold uppercase tracking-widest">Empathetic Companion</p>
            </div>
          </div>
      </div>
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-4 rounded-2xl shadow-md ${
              msg.role === 'user' 
                ? 'bg-gradient-to-br from-yellow-400 to-amber-500 text-black rounded-tr-none' 
      ...
      <div className="p-4 bg-zinc-900 border-t border-zinc-800 flex gap-2">
        <input 
            ...
            className="flex-1 bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white focus:border-yellow-400 outline-none placeholder-zinc-600"
        />
        <Button 
            label="Send" 
            onClick={sendMessage} 
